...
func (ms *cardOrderService) cacheGetCardOrder(cond *CardOrderInfo) (*response.FWError, *CardOrderInfo) {
	...
	err, data := cachehelper.KeyGet("card_order_" + strconv.FormatInt(cond.Id, 10))
	if err != nil {
		return err, nil
	}
...
func (ms *cardOrderService) cacheSetCardOrder(cardOrderInfo *CardOrderInfo) *response.FWError {
	...
	return cachehelper.KeySet("card_order_"+strconv.FormatInt(int64(cardOrderInfo.Id), 10), string(cacheData))
}

func (ms *cardOrderService) cacheDelCardOrder(cardOrderInfo *CardOrderInfo) *response.FWError {
	return cachehelper.KeyDel("card_order_" + strconv.FormatInt(int64(cardOrderInfo.Id), 10))
}
...
func (ms *cardOrderService) dbUpdateCardOrder(cardOrderInfo *CardOrderInfo) (*response.FWError, int64) {
	err, row := dbhelper.UpdateData(cardOrderInfo)
	return err, row
}
...
func (ms *cardOrderService) UpdateCardOrder(cardOrderInfo *CardOrderInfo) (*response.FWError, int64) {
	...
	err = cachehelper.KeyDel("card_order_" + strconv.FormatInt(int64(cardOrderInfo.Id), 10))
	if err != nil {
		openlog.Error("card_order_" + strconv.FormatInt(int64(cardOrderInfo.Id), 10) + " cache del failed.")
	}
	return nil, row
}
